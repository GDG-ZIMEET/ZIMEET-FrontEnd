...
import { mappingMusic, mappingStyle, mappingAge, mappingFace } from 'data/SignUpData';

interface TeamsProps {
  teamMembersInfo: TeamMemberWithProfileType[];
  setTeamMembersInfo: React.Dispatch<React.SetStateAction<TeamMemberWithProfileType[]>>;
  teamType: string;
}

const Teams: React.FC<TeamsProps> = ({ teamMembersInfo, setTeamMembersInfo, teamType }) => {
  const [teamMembers, setTeamMembers] = useState<SearchTeamMemberType[]>(teamMembersInfo);
    ...
    const fetchTeamMemberProfiles = async () => {
      try {
        if (teamMembers.length > 0) {
          const membersWithProfiles: TeamMemberWithProfileType[] = await Promise.all(
  ...
  return (
    <S.TeamLayout>
      <S.TeamContainer>
        ...
        {MyProfileData? <MyProfile profileData={MyProfileData} gender={MyProfileData.gender} isTeamMaking={true}/> : null}
        <S.Title>팀원</S.Title>
        {teamMembersInfo.map((member: TeamMemberWithProfileType) => (
          <MyProfile key={member.userId} profileData={member} gender={member.gender} isTeamMaking={true}/>)
        )}
