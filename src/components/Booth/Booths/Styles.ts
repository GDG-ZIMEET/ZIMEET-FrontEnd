...
export const Booths = styled.div<{ $selectedSpace: string }>`
  ...
  flex-wrap: wrap;
  justify-content: space-between;
  overflow-y: ${props => props.$selectedSpace === 'A' ? 'hidden' : 'auto'};
  height: 92%;
  width: 100%;
...
export const Booth = styled.div<{ $disabled: boolean; $selectedSpace: string }>`
  width: ${props => props.$selectedSpace === 'A' ? '15%' : props.$selectedSpace === 'S' ? '18%' : '21%'};
  height: ${props => props.$selectedSpace === 'A' ? '18%' : '15%'};
  margin-bottom: ${props => props.$selectedSpace === 'A' ? '2%' : '2%'};
  border-radius: 5px;
