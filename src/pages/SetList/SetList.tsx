...
const SetList = () => {
  ...
  const isCurrentPerformance = (timeRange: string, day: number) => {
    ...
    const year = 2025;
    const month = 10; // 5월
    const realDay = day === 1 ? 1 : 2; // DAY1은 1일, DAY2는 2일
  ...
  const currentPerformance = currentDayStages.find((stage) =>
    isCurrentPerformance(stage.time, stage.Day),
  );
    ...
    <>
      ...
      <S.SetListLayout>
        <S.SetListHeader
          $stageType={
            colors[
              headerStage.stageType as 'artist' | 'club' | 'other' | 'event'
            ]
          }
        >
          <S.HeaderLogoIcon />
          <S.HeaderInfoContainer>
            <S.HeaderComponent>
              ...
              <S.StageTitle $fontSize={headerStage?.font}>
                {headerStage?.name || ''}
              </S.StageTitle>
            </S.HeaderComponent>
        ...
        <S.StageListContainer>
          ...
          <S.StageList>
            {StageList.filter((stage) => stage.Day === activeDay).map(
              (stage) => {
                ...
                return (
                  <S.StageItem key={stage.id} $isActive={isActive}>
                    <S.StageColor
                      $stageType={
                        colors[
                          colorType as 'artist' | 'club' | 'other' | 'event'
                        ]
                      }
                    />
